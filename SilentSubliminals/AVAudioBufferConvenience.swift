...
extension AVAudioPCMBuffer {
        ...
        while true {
            ...
            for channel in 0 ..< channelCount {
                ...
                if blockPeak > peakValue {
                    ...
                    value.time = Double(Double(position) / Double(self.format.sampleRate))
                    peakValue = blockPeak
                }
    ...
    open func normalize() -> AVAudioPCMBuffer? {
        ...
        guard let peak: AVAudioPCMBuffer.Peak = peak() else {
            print("Failed getting peak amplitude, returning original buffer")
            return self
        }

        let gainFactor: Float = 5 / peak.amplitude
...
//            print("Error fading buffer, returning original...")
//            return self
//        }
//
